...
static void task_info_purge_list(void)
{
	...
	// For iterating + removing elements, use list_for_each_safe()
	// This works because we already passed the next list_head to q,
	// so that we can safely list_del(p)
	list_for_each_safe(p, q, &head)
	{
		// Grab a pointer for the container that contains the intrusive list pointer
		ti = list_entry(p, struct task_info, list);
		list_del(p);
		kfree(ti);
	}
